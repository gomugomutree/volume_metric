...
class volumetric:
    ...
    def __init__(self, image_address: str, npz_file: str, check_real_dist: int, n_cluster=4, resize=1):
...
############################
    ...
    def draw_image(self):
        ...
        cv2.putText(self.img, f"{(self.height*self.check_real_dist): .2f}" , (self.object_vertexes[0][0] - (self.object_vertexes[0][0]//2), (self.object_vertexes[0][1] + self.object_vertexes[1][1])//2), font, (3/self.resize) , (0, 0, 255), (10//self.resize ))
...
def main(image, npz, real_dist, time_check=False, time_check_number=10, resize=1):
    a = volumetric(image, npz, real_dist, resize=resize)
    a.set_npz_values()
